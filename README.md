...
The same aggregate durably stores the scheduler's `enabled`, `draining`, or `disabled` admission/dequeue gate. Draining lets an already-implementing fixture run settle but starts no more work; disabling while work is active rejects until the active-pause slice can prove quiescence. An operator can hold queued work before any Session or worktree is allocated. Only explicit resume clears that hold, after the current tracker issue, human readiness generation, Agent Brief, scheduler, and budget certainty are revalidated; resumptions are claimed before new work. These operations are currently caller-driven and do not register timers or pause an active Agent.
...
The admission interface accepts `startup`, `scheduled`, `manual` and authenticated-`webhook` reconciliation sources, but the bundle does not yet register timers or HTTP routes. The default configuration dispatches no model work, and no path writes to Jira. The fixture path cannot select a live provider or model. Live Jira Cloud support remains unclaimed until issue #4 receives authorized synthetic resources and credential references.
