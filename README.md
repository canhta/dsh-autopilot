...
The fixture profile exposes only its scoped `autopilot_report` tool. It masks and denies every Host-global tool, so externally configured delegation cannot create a descendant whose shutdown or token usage escapes the root run. If a required execution service is withdrawn, the dispatcher fences new work, durably requests a `service-withdrawal` pause, quiesces every owned root, and unloads; restoring the service mounts a fresh dispatcher that can resume the same retained run. Tracker reads are likewise fenced after provider withdrawal so a late result cannot mutate admission state. Scheduler timers and enabled descendant execution remain unimplemented.
